...
class StructuredControlProcess(localName: String)(implicit kernel: Kernel) extends HwProcess(localName) {
  ...
  override def entry(): Unit = {
    ...
    worker.entry {
      StructuredControl.If(worker, "Branch", 3.U > 1.U) {
        branchOut <== 7.U
      }.Else {
        branchOut <== 9.U
      }

      StructuredControl.ForRange(worker, "Loop", start = 0, endExclusive = 4, width = 8) { i =>
        acc <== acc + i
      }

      StructuredControl.While(worker, "WhileAcc", acc < 10.U) {
        acc <== acc + 2.U
      }
...
class StructuredControlSpec extends AnyFlatSpec {
  "StructuredControl" should "lower if/else, for-range, and while into working step control flow" in {
    simulate(new StructuredControlModule) { c =>
      ...
      var cycles = 0
      while (c.io.done.peek().litValue == 0 && cycles < 32) {
        c.clock.step()
        cycles += 1
      }
