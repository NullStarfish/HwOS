...
object sync {
  ...
  class MutexProcess(val maxClients: Int, localName: String)(implicit kernel: Kernel) extends HwProcess(localName) {
    ...
    class MutexLease(val id: Int) extends HwLease {
      ...
      def Unlock(): HwInline[Unit] = HwInline.stateless(s"Unlock_$id") { agent =>
        val gateLease = SysCall.Call(gate.RequestLease(id))
    ...
    def Lock(id: Int): HwInline[Unit] = HwInline.atomic(s"MutexLock_$id") { _ =>
      val lease = SysCall.Call(RequestLease(id))
      SysCall.Call(lease.Lock())
    }

    def Unlock(id: Int): HwInline[Unit] = HwInline.stateless(s"MutexUnlock_$id") { _ =>
      val lease = SysCall.Call(RequestLease(id))
  ...
  // ==========================================
  def Select(readySignals: Seq[Bool]): HwInline[UInt] = HwInline("Select") { agent =>
  ...
  class SemaphoreScoreboardProcess(
    ...
    class BusyPort(val clientId: Int) {
      def Acquire(): HwInline[Unit] = HwInline.atomic(s"AcquireBusySlot_$clientId") { t =>
